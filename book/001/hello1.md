...
* ```#include "stdafx.h"```  
   Wykorzystanie wstępnie kompliowanych nagłówków. Pliki nagłówkowe to jeden z bardziej nieudanych elementów języka, powinien być usprawniony przez moduły (wprowadzone w C++17). Wstępnie kompliowane nagłówki znakomicie przyśpieszają proces kompilacji, zostały wprowadzone w każdym szanującym się kompilatorze, w każdym inaczej. Tu: wersja Visual Studio.
* ```#include <string>```  
